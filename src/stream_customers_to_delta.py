...
from config import (
    ...
    DELTA_DEAD_LETTER_CUSTOMERS,
    CHK_CUSTOMERS,
    CHK_DEAD_LETTER_CUSTOMERS,
    HADOOP_HOME_DIR,
    SPARK_WAREHOUSE_DIR,
    SPARK_LOCAL_DIR,
    LOG_DIR,
...
log = get_logger("stream_customers", LOG_DIR)

builder = (
    SparkSession.builder
    .appName("stream_customers_to_delta")
    .master("local[*]")
    .config(
        "spark.jars",
        ",".join([
...
parsed = base.withColumn(
    "json_data",
    from_json(col("value"), customer_schema)
)
...
bad_query = (
    ...
    .format("delta")
    .outputMode("append")
    .option("path", DELTA_DEAD_LETTER_CUSTOMERS)
    .option("checkpointLocation", CHK_DEAD_LETTER_CUSTOMERS)
    .start()
)
...
good_query = (
    ...
    .format("delta")
    .outputMode("append")
    .option("path", DELTA_STAGING_CUSTOMERS)
    .option("checkpointLocation", CHK_CUSTOMERS)
    .start()
)

log.info("Customer stream started.")
